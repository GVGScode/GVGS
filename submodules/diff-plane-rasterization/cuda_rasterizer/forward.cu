...
renderCUDA(
	...
	const bool* __restrict__ gaussian_mask,
	float* __restrict__ out_accT_masked,
	const bool render_geo)
{
	...
	float accT_masked = 0.0f;  // Accumulated opacity contribution (excluding masked Gaussians)
	...
	for (int i = 0; i < rounds; i++, toDo -= BLOCK_SIZE)
	{
		...
		for (int j = 0; !done && j < min(BLOCK_SIZE, toDo); j++)
		{
			...
			// Accumulate accT_masked: if the Gaussian is not in the mask (mask is false), accumulate its contribution
			if (out_accT_masked != nullptr && gaussian_mask != nullptr) {
				if (!gaussian_mask[collected_id[j]]) {
					accT_masked += alpha * T;
				}
			}
	...
	if (inside)
	{
		...
		// Output accumulated masked opacity contribution
		if (out_accT_masked != nullptr) {
			out_accT_masked[pix_id] = accT_masked;
		}
	}
}
...
void FORWARD::render(
	...
	int* out_observe,
		...
		float* out_opacity,
		const bool* gaussian_mask,
		float* out_accT_masked,
		const bool render_geo)
{
