	...
	const torch::Tensor& gaussian_mask)
{
  ...
  torch::Tensor out_accT_masked = torch::full({H, W}, 0.0, float_opts);
  ...
  if (get_infl) {
    ...
    out_infl = torch::empty({0}, float_opts.device(device));
  }
  ...
  if(P != 0)
  {
	  ...
	  const bool* gaussian_mask_ptr = (gaussian_mask.numel() > 0) ? gaussian_mask.contiguous().data<bool>() : nullptr;
	  float* out_accT_masked_ptr = (gaussian_mask.numel() > 0) ? out_accT_masked.contiguous().data<float>() : nullptr;
  ...
  return std::make_tuple(rendered, out_color, out_infl, radii, out_observe, out_all_map, out_plane_depth, out_opacity, out_accT_masked, geomBuffer, binningBuffer, imgBuffer);
}
