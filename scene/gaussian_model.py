...
class GaussianModel:
    ...
    def save_ply(self, path, mask=None):
        ...
        with torch.no_grad():
            ...
            f_dc_tensor = self._features_dc.detach().cpu()
            f_dc = f_dc_tensor.transpose(1, 2).flatten(start_dim=1).contiguous().numpy()
            del f_dc_tensor
            f_rest_tensor = self._features_rest.detach().cpu()
            f_rest = f_rest_tensor.transpose(1, 2).flatten(start_dim=1).contiguous().numpy()
            del f_rest_tensor
    ...
    def load_ply(self, path):
        ...
        knn_f = torch.randn((xyz.shape[0], 6), dtype=torch.float, device="cuda")
        self._knn_f = nn.Parameter(knn_f.requires_grad_(True))
