...
def readColmapCameras(cam_extrinsics, cam_intrinsics, images_folder, depth_params=None):
    cam_infos = []
...
def load_depth_params(path, da3mono=False):
    """
    depth_params.json
    Args:
        path: COLMAP
        da3mono: DA3（depth_params_da3_2.json）
    Returns:
        depths_params: 
    """
    depths_params = None
    param_filename = "depth_params_da3_2.json" if da3mono else "depth_params.json"
    depth_params_file = os.path.join(path, "sparse/0", param_filename)
    if os.path.exists(depth_params_file):
        try:
            ...
            print(f"Loaded {param_filename} with {len(depths_params)} images")
        except FileNotFoundError:
            print(f"Error: {param_filename} file not found at path '{depth_params_file}'.")
        except Exception as e:
            print(f"An unexpected error occurred when trying to open {param_filename} file: {e}")
    else:
        print(f"{param_filename} not found at {depth_params_file}, using raw depth maps")
...
def readColmapSceneInfo(path, images, eval, llffhold=8, use_mvsformer_init=False, use_da3_init=False, da3mono=False):
    try:
    ...
    depths_params = load_depth_params(path, da3mono=da3mono)
    
    cam_infos_unsorted = readColmapCameras(cam_extrinsics=cam_extrinsics, cam_intrinsics=cam_intrinsics, 
                                          images_folder=os.path.join(path, reading_dir), depth_params=depths_params)
    ...
    if use_mvsformer_init:
        ply_path_mvsformer = os.path.join(path, "sparse/points3D_mvsformer.ply")
        if os.path.exists(ply_path_mvsformer):
            print(f" MVSFormer : {ply_path_mvsformer}")
            ply_path = ply_path_mvsformer
        else:
            print(f": use_mvsformer_init {ply_path_mvsformer}，")
    
    if use_da3_init:
        # ply_path_da3 = os.path.join(path, "sparse/points3D_DA3.ply")
        ply_path_da3 = os.path.join(path, "sparse/points3D_DA3_aligned.ply")
        if os.path.exists(ply_path_da3):
            print(f" DA3 : {ply_path_da3}")
            ply_path = ply_path_da3
        else:
            print(f": use_da3_init {ply_path_da3}，")
    ...
    if not os.path.exists(ply_path):
        ...
        default_ply_path = os.path.join(path, "sparse/points3D.ply")
        storePly(default_ply_path, xyz, rgb)
        if not use_mvsformer_init:
